...
1. **NEVER REMOVE THE CONVERSATIONAL Codex MODE - THIS IS THE CORE FEATURE**
   - The interactive mode in `cmd_interactive()` uses `ConversationSession` from `src/agent/conversation.py`
   - Users chat naturally with Codex who guides them through the CSES workflow
   - Codex has CSES expert knowledge embedded in the system prompt
   ...
   - If you touch `cmd_interactive()`, you MUST preserve the Codex conversation loop
...
7. **ALWAYS provide user feedback during operations**
   ...
   - Examples: "Loading data file...", "Connecting to Codex...", "Processing documents..."
...
| `src/agent/validator.py` | Codex validation logic |
...
- Keep local study folders, benchmark inputs, generated `.cses` state, and API keys out of Git.
- Never use `git add -A` in this repository. Stage explicit source, tests, wiki, and documentation paths only.
- Make small checkpoint commits after tested implementation slices.
- Before committing, run the relevant tests and inspect `git status --short` to confirm no raw study data or secrets are staged.
- Commit messages should describe the implemented behavior, not temporary debugging steps.
